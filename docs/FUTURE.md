...
- **[#299](https://github.com/webmachinelearning/webmcp/issues/299) — results when the caller aborts.** Measured and reported: `execute` receives no options argument in Chrome 152, so a tool cannot observe the abort, and it runs to completion with its writes landing while the caller is told `AbortError`. Parity is not exposed to the worst of this — no tool here loops over writes — but the mitigation is missing all the same: a page-owned cancellation path the tool checks, so a stop reports what landed. When a signal does reach `execute`, the open question is whether a *committed* booking should still be delivered to a caller who walked away. It should.
