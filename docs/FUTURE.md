...
Parity already ships answers to [#255, #262, #277, #278, #282, #286, #288, #300, #306](../README.md#designed-against-the-open-spec-issues). These are open and unanswered here:

- **[#298](https://github.com/webmachinelearning/webmcp/issues/298) — page-enforced write boundaries.** Parity's grant gate and state machine are one shape of this, but the gate is per-action. A declared *budget* — "at most one booking per session, no cancellations without a fresh grant" — is a different and probably better primitive, and it is the natural next layer above the gate.
- **[#299](https://github.com/webmachinelearning/webmcp/issues/299) — results when the caller aborts.** Parity's tools do not currently observe the abort signal at all (Chrome 152 passes `execute` a single argument, so there is no signal to observe — see the [findings](../README.md#what-we-found-in-the-browser)). When the signal arrives, the interesting question is whether a *committed* booking should still be delivered to a caller who has walked away. It should. Nothing implements that yet.
...
- **[#305](https://github.com/webmachinelearning/webmcp/issues/305) — parallel completion steps.** Relevant to the #300 deferral: if completion steps run in parallel, "defer until no tool is executing" needs to be re-tested rather than assumed.
...
|---|---|---|
| [1.4.3 Contrast (Minimum)](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum.html) | AA | Indigo on cream ≈13:1 for all body text; the peach spot colour is ≈2.2:1 and therefore decoration or ink-on-peach only, never body copy |
...
- **A real screen reader.** Everything above is verified by markup review and automated assertion. **No screen reader has been run against this build.** NVDA and VoiceOver passes are the single highest-value outstanding item, because an accessibility product whose accessibility is only inferred is making the same promise it criticises.
