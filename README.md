...
| [#300](https://github.com/webmachinelearning/webmcp/issues/300) unregistration must not fail in-flight calls | **Independently reproduced in Chrome 152, and the [WPT case](https://github.com/web-platform-tests/wpt/pull/62642) that pins it contributed upstream.** State-driven registration means a tool can unregister *itself* mid-execute; the caller was then rejected with `"The operation failed for an unknown transient reason"` after the action had already succeeded. Re-registration is now deferred whenever a sync would cut a running tool. [Details](#what-we-found-in-the-browser). |
| [#299](https://github.com/webmachinelearning/webmcp/issues/299) results on caller abort | **Measured on both sides of the fix.** Chrome 152 threads no signal, so a write loop runs to completion while the caller is told it aborted; Chromium 153 stops it at 3 of 5. The spec question survives either way: the tool's partial result is discarded, `AbortError.cause` is `undefined`, and the caller's abort *reason* never reaches the tool at all. |
