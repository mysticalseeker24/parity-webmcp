...
| [#299](https://github.com/webmachinelearning/webmcp/issues/299) results on caller abort | **Measured and reported.** The proposal presumes `execute` can observe `signal.aborted`; in Chrome 152 `execute` receives no options argument, so there is nothing to observe — and the tool runs to completion with its writes landing while the caller is told it aborted. |
| [#298](https://github.com/webmachinelearning/webmcp/issues/298) page-enforced write boundaries | Offered **enforcement by absence** as a shape alongside the proposed in-tool preconditions: a tool that is not legal now is not registered, so there is no call to refuse — paired with `unavailable[]` (#262) so absence does not destroy context. Plus grants hashed over canonical arguments, so an approval binds to *this call* rather than to the tool. |
...
| **A caller's abort cancels the report, not the work** | `executeTool(tool, args, {signal})` rejects the caller with `AbortError`, but `execute` never sees the signal (see the row above), so the tool runs to completion and its writes land. Measured: caller aborted, tool applied all five items, `<body data-applied="5">`. An agent that aborts a batch and retries has applied it twice. `AbortError.cause` is `undefined`, so there is no vehicle for attaching the tool's partial result. Reported on [#299](https://github.com/webmachinelearning/webmcp/issues/299). |
